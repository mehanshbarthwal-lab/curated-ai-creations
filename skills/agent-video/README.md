...
**Give your AI Agent the ability to watch any video.**

AI Agents (the AI Agent Code, Cursor, Antigravity, etc.) (recommended — auto-updates via marketplace):
```
/plugin marketplace add bradautomates/agent-video
/plugin install watch@agent-video
```

Codex, Cursor, Copilot, Gemini CLI, or any of 50+ [Agent Skills](https://agentskills.io) hosts:
```bash
npx skills add bradautomates/agent-video -g
```
...
An AI agent can read a webpage, run a script, browse a repo. What it can't do, out of the box, is *watch a video*. You paste a YouTube link and it has to either guess from the title or pull a transcript that's missing 90% of what's on screen.

With the AI Agent Video `/watch` you can paste a URL or a local path, ask a question, and the AI Agent fetches captions first, downloads only what it needs, extracts frames (scene-aware, or fast keyframes at `efficient` detail), pulls a timestamped transcript (free captions when available, Whisper API as fallback), and `Read`s every frame as an image. By the time it answers, it has *seen* the video and *heard* the audio.
...
**Analyze someone else's content.** `/watch https://youtu.be/<viral-video> what hook did they open with?` the AI Agent looks at the first frames, reads the opening transcript, breaks down the structure. Same for ad creative, competitor launches, podcast intros, anything where the *how* matters as much as the *what*.

**Diagnose a bug from a video.** Someone sends you a screen recording of something broken. `/watch bug-repro.mov what's going wrong?` the AI Agent watches the recording, finds the frame where the issue appears, describes what's on screen, often catches the cause without you ever opening the file.
...
3. **`ffmpeg` extracts frames at the chosen detail.** `efficient` decodes keyframes only (near-instant); `balanced`/`token-burner` prefer scene-change frames and fall back to the duration-aware uniform sampler when they under-produce. JPEGs are 512px wide by default and clamped to 1998px tall for AI Agent Read compatibility.
...
5. **Frames + transcript are handed to the AI Agent.** The script prints frame paths with `t=MM:SS` markers and the transcript with timestamps. the AI Agent `Read`s each frame in parallel — JPEGs render directly as images in its context.
6. **the AI Agent answers grounded in what's actually on screen and in the audio.** Not "based on the description" or "according to the title." It saw the frames. It heard the transcript. It answers the way someone who watched the video would.
7. **Cleanup.** The script prints a working directory at the end. If you're not asking follow-ups, the AI Agent removes it.
...
| Surface | Install |
|---------|---------|
| **AI Agents (the AI Agent Code, Cursor, Antigravity, etc.)** | `/plugin marketplace add bradautomates/agent-video` then `/plugin install watch@agent-video` |
| **Codex, Cursor, Copilot, Gemini CLI, +50 more** | `npx skills add bradautomates/agent-video -g` |
| **claude.ai** (web) | [Download `watch.skill`](https://github.com/bradautomates/agent-video/releases/latest) → Settings → Capabilities → Skills → `+` |
| **Manual / dev** | `git clone` then symlink `skills/watch` into your host's skills dir (see below) |

### AI Agents (the AI Agent Code, Cursor, Antigravity, etc.)
...
Update later with `/plugin update watch@agent-video`.
...
1. [Download `watch.skill`](https://github.com/bradautomates/agent-video/releases/latest) from the latest release.
...
```bash
git clone https://github.com/bradautomates/agent-video.git
ln -s "$(pwd)/agent-video/skills/watch" ~/.claude/skills/watch   # or ~/.codex/skills/watch
```
...
- `--resolution W` — bump frame width to 1024 px when the AI Agent needs to read on-screen text (slides, terminals, code).
...
├── hooks/                        # SessionStart status hook (AI Agents (the AI Agent Code, Cursor, Antigravity, etc.) only)
├── .claude-plugin/               # plugin.json + marketplace.json (AI Agents (the AI Agent Code, Cursor, Antigravity, etc.))
...
Built on `yt-dlp`, `ffmpeg`, and the AI Agent's multimodal `Read` tool. Whisper transcription via [Groq](https://groq.com) or [OpenAI](https://openai.com).
...
<a href="https://www.star-history.com/?repos=bradautomates%2Fagent-video&type=date&legend=top-left">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/chart?repos=bradautomates/agent-video&type=date&theme=dark&legend=top-left" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/chart?repos=bradautomates/agent-video&type=date&legend=top-left" />
   <img alt="Star History Chart" src="https://api.star-history.com/chart?repos=bradautomates/agent-video&type=date&legend=top-left" />
 </picture>
</a>

---

[github.com/bradautomates/agent-video](https://github.com/bradautomates/agent-video) · [@bradbonanno](https://www.youtube.com/@bradbonanno) · [Solaris Automation](https://www.solarisautomation.io/) · [LICENSE](LICENSE)

## Windows FFmpeg Setup (Agent Note)
If agents encounter issues where fmpeg is not found on Windows environments, they should manually download the static release from [BtbN/FFmpeg-Builds](https://github.com/BtbN/FFmpeg-Builds), extract it, and prepend the in directory containing fmpeg.exe and fprobe.exe to their shell's $env:PATH prior to invoking the skill. This guarantees the frame extraction process won't fail due to missing path variables.
